...
(function () {
    ...
    function renderTabs() {
        ...
        const tabDefs = [
            ...
            { id: 'missing', label: 'Missing Translations' },
            { id: 'stale', label: 'Stale Translations' },
    ...
    function renderOverview() {
        ...
        stats.innerHTML = `
            <div class="ept-stat"><div class="ept-stat__value">${overview.totalContent}</div><div class="ept-stat__label">Total Content</div></div>
            <div class="ept-stat"><div class="ept-stat__value">${overview.enabledLanguages.length}</div><div class="ept-stat__label">Languages</div></div>
            <div class="ept-stat"><div class="ept-stat__value">${overview.missingTranslationsCount}</div><div class="ept-stat__label">Missing Translations</div></div>
            <div class="ept-stat"><div class="ept-stat__value">${overview.staleTranslationsCount}</div><div class="ept-stat__label">Stale (30+ days)</div></div>
        `;
        ...
        header.innerHTML = '<h3>Language Coverage</h3>';
        ...
        if (overview.languageStats.length === 0) {
            EPT.showEmpty(body, 'No language data available. Run the aggregation job to collect statistics.');
        } else {
            ...
            overview.languageStats.forEach(stat => {
                ...
                langCard.innerHTML = `
                    ...
                    <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--ept-muted,#6c757d);">
                        <span>${stat.totalContent} content items</span>
    ...
    async function renderMissing() {
        ...
        toolbar.innerHTML = `
            <label class="ept-muted" style="font-size:12px;margin-right:4px;">Language:</label>
            ...
            <button class="ept-btn" id="lang-missing-tree-btn">${EPT.icons.tree} Coverage Tree</button>
        `;
    ...
    async function loadMissing() {
        ...
        try {
            ...
            stats.innerHTML = `
                <div class="ept-stat"><div class="ept-stat__value">${data.length}</div><div class="ept-stat__label">Missing ${escHtml(selectedLanguage)}</div></div>
            `;
            ...
            if (data.length === 0) {
                ...
                const body = document.createElement('div');
                body.className = 'ept-card__body';
                EPT.showEmpty(body, `All content has been translated to ${selectedLanguage}`);
                card.appendChild(body);
                content.appendChild(card);
                return;
            }
            ...
            const columns = [
                ...
                {
                    key: 'contentName', label: 'Name', render: (r) => {
                ...
                { key: 'contentTypeName', label: 'Type' },
                { key: 'masterLanguage', label: 'Master' },
                {
                    key: 'availableLanguages', label: 'Available Languages', render: (r) =>
    ...
    async function loadCoverageTree() {
        ...
        try {
            ...
            if (tree.length === 0) {
                ...
                const body = document.createElement('div');
                body.className = 'ept-card__body';
                EPT.showEmpty(body, 'No coverage tree data available');
                card.appendChild(body);
                content.appendChild(card);
                return;
            }
    ...
    async function renderStale() {
        const toolbar = document.getElementById('lang-audit-toolbar');
        toolbar.innerHTML = `
            <label class="ept-muted" style="font-size:12px;margin-right:4px;">Threshold (days):</label>
            <input type="number" id="lang-stale-threshold" class="ept-select" style="width:80px;" value="${staleThreshold}" min="1" />
            <label class="ept-muted" style="font-size:12px;margin-left:12px;margin-right:4px;">Language:</label>
            <select id="lang-stale-lang" class="ept-select">
                <option value="">All languages</option>
                ${overview.enabledLanguages.map(l => `<option value="${l}">${l}</option>`).join('')}
            </select>
            <div class="ept-toolbar__spacer"></div>
            <button class="ept-btn" id="lang-stale-apply">${EPT.icons.search} Apply</button>
        `;
    ...
    async function loadStale() {
        ...
        try {
            ...
            stats.innerHTML = `
                <div class="ept-stat"><div class="ept-stat__value">${data.length}</div><div class="ept-stat__label">Stale Translations</div></div>
            `;
            ...
            if (data.length === 0) {
                ...
                const body = document.createElement('div');
                body.className = 'ept-card__body';
                EPT.showEmpty(body, `No stale translations found (threshold: ${threshold} days)`);
                card.appendChild(body);
                content.appendChild(card);
                return;
            }
    ...
    async function renderQueue() {
        const toolbar = document.getElementById('lang-audit-toolbar');
        toolbar.innerHTML = `
            <label class="ept-muted" style="font-size:12px;margin-right:4px;">Target Language:</label>
            <select id="lang-queue-target" class="ept-select">
                ${overview.enabledLanguages.map(l => `<option value="${l}" ${l === selectedLanguage ? 'selected' : ''}>${l}</option>`).join('')}
            </select>
            <label class="ept-muted" style="font-size:12px;margin-left:12px;margin-right:4px;">Content Type:</label>
            <input type="text" id="lang-queue-type" class="ept-select" style="width:150px;" placeholder="All types" />
            <div class="ept-toolbar__spacer"></div>
            <button class="ept-btn" id="lang-queue-export">${EPT.icons.download} Export CSV</button>
        `;
    ...
    async function loadQueue() {
        ...
        try {
            ...
            stats.innerHTML = `
                <div class="ept-stat"><div class="ept-stat__value">${result.totalCount}</div><div class="ept-stat__label">Items to Translate</div></div>
                <div class="ept-stat"><div class="ept-stat__value">${result.page}/${result.totalPages || 1}</div><div class="ept-stat__label">Page</div></div>
            `;
            ...
            if (result.items.length === 0) {
                ...
                const body = document.createElement('div');
                body.className = 'ept-card__body';
                EPT.showEmpty(body, `No content needs translation to ${targetLang}`);
                card.appendChild(body);
                content.appendChild(card);
                return;
            }
            ...
            const columns = [
                ...
                { key: 'masterLanguage', label: 'Master' },
                {
                    key: 'masterLastModified', label: 'Last Updated', render: (r) => formatDate(r.masterLastModified)
                },
                {
                    key: 'availableLanguages', label: 'Available', render: (r) =>
                        r.availableLanguages.map(l => `<span class="ept-badge ept-badge--default" style="margin-right:2px;">${escHtml(l)}</span>`).join('')
                },
                {
                    key: 'breadcrumb', label: 'Location', render: (r) =>
            ...
            if (result.totalPages > 1) {
                ...
                info.textContent = `Page ${result.page} of ${result.totalPages}`;
